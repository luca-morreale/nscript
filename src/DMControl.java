 ...
 *  Takes care of handling events (keys + mouse + mouseMotion).
 *  <b>nscript</b> is structured around a Model-View-Controller Architecture
 *  DMControl represents an abstract controller which requires the
 *  existance of an abstract model (role played by DMModel) to work.
 *  Implements the logic of selection and movement of objects, which 
 *  are stored in the model, including the use of key modifiers like
 * shift.
 */
    ...
    implements MouseListener, MouseMotionListener
{
    ...
    */
    DMModel M;
    /** A reference where a link object starts, for the case when
    ...
    /** The only constructor requires references to the view object
    ...
    /** A function that returns which of the model objects is hit my a
     *  mouse click.
     ...
     *  @param index to the desired object. index must be between 1 and DMControl.getSelectedCount().
     *  @return a reference to the object, if the index is correct, <b>Null</b> otherwise.
     */
    ...
    /** Processes the click event, where the user presses and releases the mouse button
     *  in the same location. Implementation of the mouseListener interface.
     ...
     *  is working on. Specifically, it responds to SELECT_MODE, by preparing 
     *  for a movement of the selected items if one of them is hit, when no
     *  <shift> key is pressed. If <shift> key is pressed, the response is to
     *  start a dragging action to create a rectangle. The 'selected' state of all 
    ...
    /** Draws an auxiliary figure to aid the user in the editing process. It draws
     *  the line partially being drawn when creating a new link between objects, or
     *  a rectangle, when a selection rectangle is created. 
     *  @param g a Graphics object with the information about how to draw on the screen
     *  provided by the calling routine (in this case DMView).
     */
    ...
    /** Process the mouse released event that occurs when the user
     *  has finished its dragging action. This can happen when 
     *  the editing mode is SELECTING, MOVING, or LINKING (creation
     *  of a link between to iconic objects, such as a network link).
    ...
    /** Reponse to the movement of the mouse.  Only relevant if
     *  a link is being created (LINKING), a set of selected objects
     *  is being moved (MOVING), or a selection rectangle is being drawn (SELECTING).
     *  @param me a MouseEvent with the information about the mouse position and state.
     */
    ...
    /** Implementation of selected key events. Specifically the 'backspace' key that
     *  marks object removal.
