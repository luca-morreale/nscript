...
public class SToolBar extends JPanel implements ActionListener
{
    ...
    NScript frame;
    /** A reference to a Library Manager which stores libraries and their information. */
    TclLibraryManager libManager;
    /** A tabbed pane. This interface elements contains a "tab" for each library that is
     *  opened. */
    JTabbedPane tabPane;
	...
	/** Constructor that takes the parent frame, and a reference to the library manager.
	 *  Creates and initializes the interface elements. 
	...
	JButton b = new JButton(new ImageIcon("figs/select.gif"));
	...
	/** Adds a new library to the interface by creating a new tabbed pane, and configuring
	 *  it to store that libraries classes.
	...
	/** Returns a reference to the class currently selected by the user (TclSnippet). 
    ...
    /** Clears the selection in the current pane. This is a hack in response of the user selecting
     *  the "Select" tool. */
	...
	/** Implements the ActionListener interface to respond to the "Select" button. 
	...
	/** Class that implements the AbstractListModel behavior to render the elements of a library as
	 *  a list. */
	...
	/** Obtains the size of a library in terms of number of classes (TclSnippet). 
