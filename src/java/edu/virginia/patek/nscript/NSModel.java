...
/** Holds the current simulation script information. Inherits the object storing capabilities
 *  from NSWorld, and implements the DMModel interface, in order to work correctly with the
 *  DMControl, and DMView objects. Also contains references to all the views of the model to
 *  inform them of model changes. */
public class NSModel extends NSWorld implements DMModel
{
    ...
    DMView editView;
    /** Reference to the view where the Tcl version of the simulation script is represented. */
    JTextArea tclView;
    /** Reference to the panel where individual objects can be edited. */
    SObjectBrowser objectPanel;
    /** Reference to the view that lists all the objects in the simulation (not yet implemented). */
    NSWorldView worldView;
    /** Flag that tells if the model has been modified and needs to be saved to a file. */
    boolean isDirty;
    
    /** The onlye constructor for this class of objects. Takes an initial environment object
     *  (the ns object, that stores the simulation environment), and a reference to the 
     *  toolbox. */
    ...
    /** Returns the current edition mode for the model (perhaps this should be moved to the view). 
     ...
     *  @param unselectOther if true all other objects should be unselected after adding the new object. */
     ...
     *  @param unSelectOthers if true the editor all other objects should be unselected after
    ...
    /** Tells all its dependent views to update their representation, as a response to a change in the
     *  model information. */
    ...
    /** Creates a new model by flushing all the stored objects, and initializing the
     *  ns environment object to its default values. */
    ...
    /** Creates a Tcl representation of the current object. This call creates the TclView used in the interface. 
    ...
    /** Represents the current simulation model as a string. This representation is used to store the model
     *  on disk. 
    ...
    /** True if the model needs to be saved to disk (changes have been made to it since it was opened). 
