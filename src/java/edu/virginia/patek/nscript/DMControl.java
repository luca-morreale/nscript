...
public class DMControl extends KeyAdapter
  ...
  */
   ...
   *  @return a reference to the object, if the index is correct,
   *          <b>Null</b> otherwise.
   */
   ...
   *  mouse button in the same location. Implementation of the mouseListener
   *  interface.
  ...
  /** Process the event of the button being pressed, and prepares, if
   *  necessary for a dragging action. The actual behavior depends on the
   *  mode the Model is working on. Specifically, it responds to SELECT_MODE,
   *  by preparing for a movement of the selected items if one of them is hit
   *  when no <shift> key is pressed. If <shift> key is pressed, the response
   *  is to start a dragging action to create a rectangle. The 'selected'
   *  state of all elements of the model encompassed by the resulting
  ...
  /** Draws an auxiliary figure to aid the user in the editing process.
   *  It draws the line partially being drawn when creating a new link
   *  between objects, or a rectangle, when a selection rectangle is created.
   *  @param g a Graphics object with the information about how to draw on
   *           the screen provided by the calling routine
   *           (in this case DMView).
   */
   ...
   *  is being moved (MOVING), or a selection rectangle is being drawn
   *  (SELECTING).
   *
  ...
  /** Implementation of selected key events. Specifically the 'backspace'
   *  key that marks object removal.
   *
