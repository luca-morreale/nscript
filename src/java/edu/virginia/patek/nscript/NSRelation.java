...
/** Implements the generic behavior of a relation element on the diagram. These objects
 *  are used to represent links, attach, and connect operations. */
class NSRelation extends NSEditableObject implements DMObject
{
    ...
    /** A routine where the relation draws itself on the provided graphic context (Graphics), and
     *  using the dimension of the current view pane. It uses class definition information to
     *  select the drawing attributes, such as line width, and the style of the beginning and ending
     *  of the line. 
    ...
    /** Translates the object by translating the objects that it relates. To achieve this
     *  it selects the from-to objects is they were not selected to move. This can be seen
     *  during the edition. Selecting a link and moving it around the view pane automatically
     *  selects the entities that define its extremes. 
    ...
    /** Returns true if the provided coordinate is close enough to the line that represents the relation.
     *  @param r the size of the view pane. 
	...
	y2 = getTo().getY();
        ...
        // If F is the origin point of the line, and D the destination point, and P
        // the provided point, this procedures calculates the projection of vector
        // (P-F) into the vector (D-F), this projection is then expressed as a multiple
        // (t0) of the (D-F) vector. If t0 is in the interval [0,1], and the distance 
        // to the line is less than the radius, return true, otherwise false. 
    ...
    /** Returns true if the relation is contained in the rectangle delimited by the
     *  provided corners (2). Simply checks if both of the provided points are contained
     *  in the rectangle.
     *  @param r the size of the view pane. 
     *  @p1 the first point (corner) of a rectangle. 
     *  @p2 the opposite corner of the rectangle. 
     *  @return true is both extremes of the relation are contained, false otherwise. */
    ...
    /** Creates a representation of the relation object as a String. Used to store
     *  the object on disk. 
     ...
     *  origin and destination objects, which are assumed to exist in memory
     *  by the time an attempt to create relation between them is made.
