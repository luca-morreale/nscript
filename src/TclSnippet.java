 ...
 *  about a class of objects, such as the attributes that it contains, their default values and 
 *  options. It also contains information on how to translate this information into a Tcl script. */
public class TclSnippet extends Object
{
	/** The name of the snippet. */
    String name;
    /** Name of the base class. A snippet belongs to a given category, to provide more
     *  flexibility. For example, TCP, TCPSink, UDP, and Null share the 'agent' base class. */
    String base;
     ...
     *  there can only be one ns environment class per simulation model. Also there
     *  can only be a unique application attached to an agent. */
    public boolean isUnique;
    
    /** The icon used to represent objects of this class in the graphic environment. */
    int icon;
    /** Describes the class base required to create a relation of this type. It applies only
     *  to relation objects. For example a DuplexLink object can only connect to node objects. 
     *  In this case describes the base class of the object where the relation starts. */
    String fromBase;
    /** Describes the class base of the object where the relation ends. */
    String toBase;
    
    /** Flags if the relation start must be unique. This is that the start object can only
     *  accept one relation of this class, and applies only to relation objects. */
    public boolean isFromBaseUnique;
    /** Flags if the relation end must be unique. This means that the end object can only
     *   accept one relation of this class, and applies only to relation objects. */
    public boolean isToBaseUnique;
    
    /** The style of the base of the relation (start of the relation), so that objects can
     *  be drawn with different arrow styles. */
    int baseStyle; 
	/** The style of the line for a relation object. This is if the line should be drawn 
	 *  solid, dotted, or dashed. */
    int lineStyle; 
    /** The width of the line for relation objects. */
    int lineWidth; 
    /** The style of the end of the relation, so that relation objects can be drawn with different
     *  arrows styles. */
    int endStyle;
	...
	/** Method that parses the attributes and patterns for the snippet from a String object. 
	...
	/** Gets the base class.
	 *  @param inBase a string containing the class name of the base class object. */
    public void setBase( String inBase )
    {
		base = inBase;
    }

	/** Obtains the name of the base class object. Applies only to relation objects. 
	...
	/** Sets the icon to be used by the objects of this class. Applies only to entity objects. 
	...
	/** Sets the base class for the objects where an instance of this class can start. 
	 *  Applies only to relation objects. 
	...
	/** Instantiates an object by copying the default values of the attributes to it. 
	 ...
	 *  @param w a reference to the world object. This reference is provided in case
	 *  the object requires context information to resolve its translation. 
	 *  @param o the object to be rendered to Tcl. 
	 *  @param sep a character that separates the inline expressions in the patterns. 
    ...
    public String toTcl( NSWorld w, NSObject o, char sep )
    {
		...
		if (o.getArrayIndex()>=0) {
			...
			if (isRelation) {
			ro = (NSRelation) o;
			i = ro.getFrom().getArrayIndex(); i2 = ro.getTo().getArrayIndex();
			if (i>=0 || i2>=0) {
				if (i==i2) {
