 ...
 *  a function to discover if the object is 'engulfed' in a rectangle. 
...
{
    /** The x coordinate of the position of the object. x is in the [0,1] interval. */
    double x;
    /** The y coordinate of the position of the object. y is in the [0,1] interval. */
    double y;
	...
	double scaleY = r.height;
        ...
        // Change the color to blue if the object has the 'Selected' status (i.e. it is being
        // modified.
	if (isSelected()) {
	    color = g.getColor();
        ...
        // Transform the position of the object from the interval [0,1] using the height
        // and width of the view pane.
	...
	int size2 = (int) Math.round( size / 2 );
        ...
        // If the element is indexed by an array, display it with a 'shadow' to give
        // the impresion of a stack of objects.
	if (arrayIndex>=0) {
    ...
    /** True if the position of the object is 'close' to the given mouse point. It is
    ...
    /** Returns true if the element is contained in the rectangle contained in the 
     *  rectangle described by the points given. 
     *  @param r the size of the view pane.
     *  @param p1 the first point of the rectangle.
     *  @param p2 the second point marking the rectangle. 
     *  @return true if the element is contained by the rectangle, false otherwise. */
    ...
    /** Reads the element from a budderedReader (stream), basically reading its coordinates. 
     *  The rest of the information is carried on higher levels of the hierarchy. 
     *  @param br the stream that contains the object information, tipically a file. */
