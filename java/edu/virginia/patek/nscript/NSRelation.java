 ...
 *  These objects are used to represent links, attach, and connect operations.
 */
class NSRelation extends NSEditableObject implements DMObject {
   ...
   *  qualifies a click as a selection. In these case the radios is in terms
   *  of the width of the line. The default value is 2.0 which means that
   *  if a line has a width of 1 pixel, the selection tolerance is 2 pixels.
   */
  public static final double SELECTION_RADIUS = 5.0;

  /** Requires a base class definition (TclSnippet), a name, and the origin
   *  and destination entity that it relates.
   *
  ...
  /** A routine where the relation draws itself on the provided graphic
   *  context (Graphics), and using the dimension of the current view pane.
   *  It uses class definition information to select the drawing attributes,
   *  such as line width, and the style of the beginning and ending of the
   *  line.
   *
  ...
  /** Translates the object by translating the objects that it relates.
   *  To achieve this it selects the from-to objects is they were not
   *  selected to move. This can be seen during the edition. Selecting a
   *  link and moving it around the view pane automatically selects the
   *  entities that define its extremes.
   *
  ...
  /** Returns true if the provided coordinate is close enough to the line
   *  that represents the relation.
   *
   *  @param r the size of the view pane.
  ...
  /** Returns true if the relation is contained in the rectangle delimited
   *  by the provided corners (2). Simply checks if both of the provided
   *  points are contained in the rectangle.
   *
   *  @param r the size of the view pane.
  ...
  /** Creates a representation of the relation object as a String.
   *  Used to store the object on disk.
   *
